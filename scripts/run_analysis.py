...
for repo in repo_list:
    ...
    subprocess.run(
        ["git", "clone", "--recurse-submodules", clone_url, str(repo_path)],
        check=True
    )
    ...
    print(f"Default branch for {repo}: {result.stdout.strip()}")

    result = subprocess.run(
        ["git", "rev-parse", "HEAD"],
        cwd=repo_path, text=True, capture_output=True, check=True
    )
    print(f"Latest commit for {repo}: {result.stdout.strip()}")
    ...
    cmd = [
        "python", str(script_copy),
        "--no-reference-xlsx",
        "--output", str(output_file)
    ]

    submodule_args = ORG_SUBMODULE_TARGETS.get(repo, [])  # ← Fixed: was ORG
    if submodule_args:
    ...
    print(f"Report generated: {output_file}")  # ← Fixed: was mis-indented
